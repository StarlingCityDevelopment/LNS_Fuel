...
local function CleanUpJob()
	if activeJob then
		...
		if activeJob.returnBlip and DoesBlipExist(activeJob.returnBlip) then
			RemoveBlip(activeJob.returnBlip)
		end
		activeJob = nil
	end
	hideDeliveryUI()
...
local function UnloadFuel()
	if not activeJob or activeJob.state ~= "deliver" then
		return
	end
...
local function ReturnTruck()
	if not activeJob or activeJob.state ~= "return" then
		return
	end
		...
		if activeJob and not activeJob.spawning then
			...
			if activeJob.state == "tow" then
				...
				if truckExists and trailerExists then
					if
						activeJob.truck
						and not activeJob.truckCollected
						and GetPedInVehicleSeat(activeJob.truck, -1) == cache.ped
					then
					...
					if dist < 5.0 then
						...
						if veh ~= 0 and veh == activeJob.truck then
							if IsVehicleAttachedToTrailer(veh) then
								showDeliveryUI(
									locale("textui_unload_fuel"),
									{ position = "right-center", icon = "gas-pump" }
								)
								if IsControlJustPressed(0, 38) then
									hideDeliveryUI()
									UnloadFuel()
								...
								showDeliveryUI(
									locale("textui_attach_trailer"),
									{ position = "right-center", icon = "exclamation-circle" }
								)
							end
						else
							showDeliveryUI(
								locale("textui_must_be_in_hauler"),
								{ position = "right-center", icon = "exclamation-circle" }
							)
						end
					else
