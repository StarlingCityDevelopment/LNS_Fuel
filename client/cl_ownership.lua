...
function cl_ownership.getStationName(stationId, isElectric)
	...
	return isElectric and (locale("charging_station_blip") or "Public EV Charging Station")
		or locale("fuel_station_blip")
end
...
local function isStationElectric(stationId)
	...
	for i = 1, #fuelStations do
		...
		if
			sid == stationId
			and station.cantBeOwned
			and (station.electricpumps ~= nil or station.electricPumps ~= nil)
		then
			return true
		end
	end
...
RegisterNetEvent("LNS_Fuel:syncStation", function(stationId, data)
	...
	if blip then
		...
		local name = data and data.name
			or (isElectric and locale("charging_station_blip") or locale("fuel_station_blip"))
...
function cl_ownership.promptPurchase(stationId)
	local alert = lib.alertDialog({
		...
		content = ("Are you sure you want to purchase this gas station for $%s?"):format(
			Settings.ownership.defaultPurchasePrice
		),
		centered = true,
		cancel = true,
	})
