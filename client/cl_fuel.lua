...
local function getPumpTypeNearPlayer()
	...
	if Settings.pumpModels then
		for i = 1, #Settings.pumpModels do
			...
			local entity =
				GetClosestObjectOfType(plyCoords.x, plyCoords.y, plyCoords.z, 3.0, model, false, false, false)
...
function fuelModule.startFueling(veh, usingPump)
	...
	if missing < Settings.refillValue then
		local errorMsg = isElectricVeh
				and (locale("battery_full") or "The battery of this vehicle is already fully charged!")
			or locale("tank_full")
	...
	if usingPump then
		...
		if isElectricVeh and not isElectricPump then
			lib.notify({
				type = "error",
				description = locale("electric_vehicle_gas_pump")
					or "This vehicle is electric! You must use a charging station.",
			})
			return
		end
		if not isElectricVeh and isElectricPump then
			lib.notify({
				type = "error",
				description = locale("not_electric_vehicle")
					or "This vehicle is not electric! You must use a gas station.",
			})
			return
		end
	else
		if isElectricVeh then
			lib.notify({
				type = "error",
				description = locale("electric_vehicle_no_jerrycan")
					or "You cannot charge an electric vehicle with a petrol can!",
			})
			return
		end
	end
			...
			if sConf then
				...
				if not stationId and sConf.electricpumps then
					for _, pump in ipairs(sConf.electricpumps) do
						...
						local coords = type(rawCoords) == "table"
								and vec3(
									rawCoords.x or rawCoords[1],
									rawCoords.y or rawCoords[2],
									rawCoords.z or rawCoords[3]
								)
							or rawCoords
	...
	if usingPump then
		...
		CreateThread(function()
			...
			local progressLabel = isElectricVeh and (locale("progress_charging") or "Charging vehicle...")
				or locale("progress_refueling")
			hlpr.progress({
...
local function setupHosePhysicalRope(pumpObj, pumpCoords, nozzleProp, isElectricType)
	...
	local offsetNozzle = GetOffsetFromEntityInWorldCoords(
		nozzleProp,
		isElectricType and -0.005 or 0.0,
		isElectricType and 0.185 or -0.033,
		isElectricType and -0.05 or -0.195
	)
	...
	CreateThread(function()
		...
		while st.holdingNozzle or st.holdingElectricNozzle do
			...
			if currentDist > maxRange or cache.vehicle then
				...
				lib.notify({
					type = "error",
					description = locale("nozzle_cannot_reach") or "The hose cannot reach this far!",
				})
				break
			end
			Wait(1000)
...
function fuelModule.returnNozzle()
	if not (st.holdingNozzle or st.holdingElectricNozzle) then
		return
	end
