...
local function spawnStationChargers(stationId, chargers)
	...
	if not chargers or #chargers == 0 then
		return
	end
...
local function RotationToDirection(rotation)
	local adjustedRotation =
		vec3((math.pi / 180.0) * rotation.x, (math.pi / 180.0) * rotation.y, (math.pi / 180.0) * rotation.z)
	return vec3(
			...
			if now - lastUpdate > 30 then
				...
				if inRange ~= lastInRange then
					...
					if inRange then
						...
						lib.showTextUI(
							locale("textui_too_far_station"),
							{ position = "bottom-center", icon = "triangle-exclamation" }
						)
					end
				end
			end
		...
		if chargers then
			...
			for j = 1, #chargers do
				...
				local coords = type(rawCoords) == "table"
						and vec3(rawCoords.x or rawCoords[1], rawCoords.y or rawCoords[2], rawCoords.z or rawCoords[3])
					or rawCoords
