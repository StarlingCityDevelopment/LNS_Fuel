...
exports.ox_target:addModel(Settings.pumpModels, {
    ...
    },
    {
        ...
        canInteract = function(entity)
            return Settings.petrolCan.enabled and not st.holdingNozzle and not st.holdingElectricNozzle and not st.isFueling
        end
    }
})
...
exports.ox_target:addGlobalVehicle({
    ...
    },
    {
        ...
        onSelect = function(targetData)
            if GetVehicleFuelLevel(targetData.entity) >= 100 then
                lib.notify({ type = 'error', description = locale('battery_full') or "The battery of this vehicle is already fully charged!" })
                return 
            end
            fuelMod.startFueling(targetData.entity, true)
        end,
        canInteract = function(entity)
            return st.holdingElectricNozzle and not st.isFueling and not cache.vehicle and DoesVehicleUseFuelSafe(entity)
        end
    }
})
