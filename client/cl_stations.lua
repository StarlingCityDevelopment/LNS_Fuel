...
if Settings.ox_target and Settings.showBlips ~= 1 then
	return
end
...
local function handleStationProximity(pData)
	if pData.currentDistance > 15 then
		return
	end
	...
	local allPumps = {}
	for i = 1, #availablePumps do
		allPumps[#allPumps + 1] = availablePumps[i]
	end
	for i = 1, #electricPumps do
		local coords = type(electricPumps[i]) == "table" and electricPumps[i].coords or electricPumps[i]
		local vec3Coords = type(coords) == "table"
				and vec3(coords.x or coords[1], coords.y or coords[2], coords.z or coords[3])
			or coords
