...
SetFuelConsumptionRateMultiplier(
	startVeh ~= 0 and GetVehicleFuelConsumptionRate(startVeh) or Settings.globalFuelConsumptionRate
)
...
local function handleVehicleDriving()
	...
	if not veh or veh == 0 or not DoesEntityExist(veh) or not DoesVehicleUseFuel(veh) then
		return
	end
...
if cache.seat == -1 then
	CreateThread(handleVehicleDriving)
end
...
RegisterCommand("startfueling", function()
	...
	if nearPump then
		...
		if not isVehNear then
			if not Settings.petrolCan.enabled then
				return
			end
