...
if not Settings or not Settings.ownership or not Settings.ownership.enabled then
	return
end
	...
	if results then
		for _, row in ipairs(results) do
			...
			if row.upgrades and row.upgrades ~= "" then
				pcall(function()
					upgrades = json.decode(row.upgrades) or upgrades
				end)
			end
			if row.statistics and row.statistics ~= "" then
				pcall(function()
					stats = json.decode(row.statistics) or stats
				end)
			end
			...
			if row.electric_chargers and row.electric_chargers ~= "" then
				pcall(function()
					electric_chargers = json.decode(row.electric_chargers) or electric_chargers
				end)
			end

			local emps = {}
			local empResults = MySQL.query.await(
				"SELECT identifier, name, role FROM lns_fuel_employees WHERE station_id = ?",
				{ row.station_id }
			)
			if empResults then
...
local function saveStation(stationId)
	...
	local station = ownedStations[stationId]
	if not station or station.pending then
		return
	end
...
lib.callback.register("LNS_Fuel:buyStation", function(source, stationId)
	...
	local plyId = getPlayerIdentifier(source)
	if not plyId then
		return false, locale("notify_cannot_identify_player")
	end
	...
	if not idx then
		reportSecurityCheck(
			source,
			("[Security Check] Player %s attempted to buy invalid stationId %s"):format(source, stationId)
		)
	...
	if stationConf and stationConf.cantBeOwned then
		reportSecurityCheck(
			source,
			("[Security Check] Player %s attempted to buy non-ownable stationId %s"):format(source, stationId)
		)
	...
	if not isNearStationManagement(source, stationId, 20.0) then
		reportSecurityCheck(
			source,
			("[Security Check] Player %s attempted to buy stationId %s too far away!"):format(source, stationId)
		)
	...
	lib.logger(
		source,
		"Station Purchased",
		("Purchased fuel station %s for $%d"):format(stationId, cost),
		("stationId:%s"):format(stationId),
		("cost:$%d"):format(cost)
	)
...
lib.callback.register("LNS_Fuel:getStationManagement", function(source, stationId)
	...
	if not isNearStationManagement(source, stationId, 20.0) then
		reportSecurityCheck(
			source,
			("[Security Check] Player %s attempted to access station management for %s too far away!"):format(
				source,
				stationId
			)
		)
	...
	if not isOwner and not isEmp then
		return nil
	end
...
RegisterNetEvent("LNS_Fuel:renameStation", function(stationId, newName)
	...
	if not isNearStationManagement(src, stationId, 20.0) then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s attempted to rename station %s too far away!"):format(src, stationId)
		)
		return
	end
	...
	for _, word in ipairs(Settings.ownership.blacklistWords) do
		if string.find(cleanName, word:lower()) then
			return TriggerClientEvent(
				"ox_lib:notify",
				src,
				{ type = "error", description = locale("notify_blacklisted_word") }
			)
		end
	end
	...
	lib.logger(
		src,
		"Station Renamed",
		('Renamed station %s to "%s"'):format(stationId, newName),
		("stationId:%s"):format(stationId),
		("name:%s"):format(newName)
	)
...
RegisterNetEvent("LNS_Fuel:withdrawMoney", function(stationId, amount)
	...
	if not isNearStationManagement(src, stationId, 20.0) then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s attempted to withdraw money from station %s too far away!"):format(
				src,
				stationId
			)
		)
	...
	if not validatedAmount or validatedAmount <= 0 then
		return
	end
	...
	if station.balance < validatedAmount then
		return TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "error", description = locale("notify_not_enough_funds") }
		)
	end
	...
	lib.logger(
		src,
		"Station Withdraw",
		("Withdrew $%d from station %s balance"):format(validatedAmount, stationId),
		("stationId:%s"):format(stationId),
		("amount:$%d"):format(validatedAmount)
	)

	TriggerClientEvent(
		"ox_lib:notify",
		src,
		{ type = "success", description = locale("notify_withdraw_success"):format(validatedAmount) }
	)
end)
...
RegisterNetEvent("LNS_Fuel:depositMoney", function(stationId, amount)
	...
	if not isNearStationManagement(src, stationId, 20.0) then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s attempted to deposit money into station %s too far away!"):format(
				src,
				stationId
			)
		)
	...
	if playerCash < validatedAmount then
		return TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "error", description = locale("notify_not_enough_cash") }
		)
	end
	...
	lib.logger(
		src,
		"Station Deposit",
		("Deposited $%d into station %s balance"):format(validatedAmount, stationId),
		("stationId:%s"):format(stationId),
		("amount:$%d"):format(validatedAmount)
	)
	...
	TriggerClientEvent(
		"ox_lib:notify",
		src,
		{ type = "success", description = locale("notify_deposit_success"):format(validatedAmount) }
	)
end)
...
RegisterNetEvent("LNS_Fuel:setPrice", function(stationId, price)
	...
	if not isNearStationManagement(src, stationId, 20.0) then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s attempted to set price for station %s too far away!"):format(src, stationId)
		)
		return
	end

	local validatedPrice = validateInteger(price)
	if not validatedPrice then
		return
	end
	...
	if validatedPrice < minPrice or validatedPrice > maxPrice then
		return TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "error", description = locale("notify_price_range"):format(minPrice, maxPrice) }
		)
	end
	...
	lib.logger(
		...
		("Set fuel price to $%d at station %s"):format(validatedPrice, stationId),
		("stationId:%s"):format(stationId),
		("price:$%d"):format(validatedPrice)
	)

	TriggerClientEvent(
		"ox_lib:notify",
		src,
		{ type = "success", description = locale("notify_price_set_success"):format(validatedPrice) }
	)
end)
...
RegisterNetEvent("LNS_Fuel:setElectricPrice", function(stationId, electricPrice)
	...
	if not isNearStationManagement(src, stationId, 20.0) then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s attempted to set electric price for station %s too far away!"):format(
				src,
				stationId
			)
		)
	...
	lib.logger(
		src,
		"Station Electric Price Change",
		("Set electric price to $%d at station %s"):format(validatedPrice, stationId),
		("stationId:%s"):format(stationId),
		("electricPrice:$%d"):format(validatedPrice)
	)
...
RegisterNetEvent("LNS_Fuel:orderStock", function(stationId, orderIndex, isAuto)
	...
	if not isNearStationManagement(src, stationId, 20.0) then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s attempted to order stock for station %s too far away!"):format(src, stationId)
		)
		return
	end
	...
	if activeDeliveries[stationId] or station.activeDelivery then
		return TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "error", description = locale("notify_delivery_in_progress") }
		)
	end

	local validatedIndex = validateInteger(orderIndex)
	if not validatedIndex then
		return
	end
	...
	if station.stock + orderData.amount > station.capacity then
		return TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "error", description = locale("notify_stock_exceeds_capacity") }
		)
	end
	...
	if isAuto then
		...
		if not hiredDriverUpgrade or not hiredDriverUpgrade.enabled then
			return TriggerClientEvent(
				"ox_lib:notify",
				src,
				{ type = "error", description = locale("notify_upgrade_disabled") or "Hired drivers are disabled." }
			)
		end
		...
		if hiredDriverLvl == 0 then
			return TriggerClientEvent(
				"ox_lib:notify",
				src,
				{ type = "error", description = locale("notify_requires_dispatch_upgrade") }
			)
		end
	...
	if station.balance < totalCost then
		return TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "error", description = locale("notify_not_enough_balance_delivery") }
		)
	end
	...
	if not stationCoords then
		return TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "error", description = locale("notify_coords_not_found") }
		)
	end
	...
	if isAuto then
		...
		lib.logger(
			src,
			"Station Stock Ordered",
			("Dispatched AI driver to deliver %d stock for station %s (cost: $%d)"):format(
				orderData.amount,
				stationId,
				totalCost
			),
			("stationId:%s"):format(stationId),
			("amount:%d"):format(orderData.amount),
			("cost:$%d"):format(totalCost),
			"isAuto:true"
		)
		...
		TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "info", description = locale("notify_ai_delivery_dispatched") }
		)
			...
			if currentDelivery and currentDelivery.isAuto and currentDelivery.startTime == deliveryObj.startTime then
				...
				if currentStation then
					...
					lib.logger(
						...
						"Station Delivery Completed",
						("AI driver completed delivery of %d stock for station %s"):format(finalAmount, stationId),
						("stationId:%s"):format(stationId),
						("amount:%d"):format(finalAmount),
						"isAuto:true"
					)

					if GetPlayerPing(currentDelivery.source) > 0 then
						TriggerClientEvent("ox_lib:notify", currentDelivery.source, {
							type = "success",
							description = locale("notify_ai_delivery_completed"):format(finalAmount),
						})
					end
				end
			end
		...
		lib.logger(
			src,
			"Station Stock Ordered",
			("Player started manual delivery run of %d stock for station %s (cost: $%d)"):format(
				orderData.amount,
				stationId,
				totalCost
			),
...
RegisterNetEvent("LNS_Fuel:completeDelivery", function(stationId)
	...
	if elapsed < 10 then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s attempted LNS_Fuel:completeDelivery too fast! Elapsed: %d seconds. Action blocked."):format(
				src,
				elapsed
			)
		)
	...
	if stationCoords then
		...
		if dist > 35.0 then
			reportSecurityCheck(
				src,
				("[Security Check] Player %s attempted LNS_Fuel:completeDelivery for station %s from a distance of %0.2f meters! Action blocked."):format(
					src,
					stationId,
					dist
				)
		...
		if delivery.trailerNetId then
			...
			if DoesEntityExist(trailer) then
				...
				if distTrailer > 35.0 then
					reportSecurityCheck(
						src,
						("[Security Check] Player %s attempted LNS_Fuel:completeDelivery but trailer is too far from station! Distance: %0.2f meters"):format(
							src,
							distTrailer
						)
					)
				...
				if Settings.spawnType == "server" then
					reportSecurityCheck(
						src,
						("[Security Check] Player %s attempted LNS_Fuel:completeDelivery but trailer entity does not exist on server!"):format(
							src
						)
					)
					return
	...
	if station and not station.pending then
		...
		lib.logger(
			src,
			"Station Delivery Completed",
			("Completed manual delivery run of %d stock for station %s"):format(finalAmount, stationId),
			("stationId:%s"):format(stationId),
			("amount:%d"):format(finalAmount),
			"isAuto:false"
		)
		...
		TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "success", description = locale("notify_delivery_completed"):format(finalAmount) }
		)
	end
...
RegisterNetEvent("LNS_Fuel:buyUpgrade", function(stationId, upgradeType)
	...
	if not isNearStationManagement(src, stationId, 20.0) then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s attempted to buy upgrade for station %s too far away!"):format(src, stationId)
		)
		return
	end
	...
	if upgradeType == "hiredDriver" and not upgradeConf.enabled then
		return TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "error", description = locale("notify_upgrade_disabled") or "This upgrade is currently disabled." }
		)
	end
	...
	if station.balance < levelData.price then
		return TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "error", description = locale("notify_not_enough_balance_upgrade") }
		)
	end
	...
	lib.logger(
		src,
		"Station Upgrade Purchased",
		("Purchased station upgrade %s to level %d for $%d"):format(upgradeType, nextLevel, levelData.price),
		("stationId:%s"):format(stationId),
		("upgradeType:%s"):format(upgradeType),
		("level:%d"):format(nextLevel),
		("cost:$%d"):format(levelData.price)
	)

	TriggerClientEvent(
		"ox_lib:notify",
		src,
		{ type = "success", description = locale("notify_upgrade_success"):format(upgradeConf.title, nextLevel) }
	)
end)
...
RegisterNetEvent("LNS_Fuel:sellStation", function(stationId)
	...
	if not isNearStationManagement(src, stationId, 20.0) then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s attempted to sell station %s too far away!"):format(src, stationId)
		)
		return
	end
	...
	lib.logger(
		src,
		"Station Sold",
		("Sold station %s for a total refund of $%d"):format(totalRefund, totalRefund),
		("stationId:%s"):format(stationId),
		("refund:$%d"):format(totalRefund)
	)

	TriggerClientEvent(
		"ox_lib:notify",
		src,
		{ type = "success", description = locale("notify_station_sold"):format(totalRefund) }
	)
end)
...
RegisterNetEvent("LNS_Fuel:addCharger", function(stationId, coords, heading)
	...
	if not isNearStationManagement(src, stationId, 20.0) then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s attempted to add charger to station %s too far away!"):format(src, stationId)
		)
		return
	end
	...
	if #station.electric_chargers >= maxLimit then
		return TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "error", description = "You have reached your power grid capacity limit!" }
		)
	end
	...
	if #(coords - center) > maxRange then
		return TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "error", description = "Charger placed outside of the station's zone!" }
		)
	end
	...
	if station.balance < price then
		return TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "error", description = "Insufficient funds in the business vault!" }
		)
	end
	...
	lib.logger(
		src,
		"Station Charger Purchased",
		("Purchased and placed electric charger at station %s for $%d"):format(stationId, price),
		("stationId:%s"):format(stationId),
		("cost:$%d"):format(price)
	)
	...
	TriggerClientEvent(
		"ox_lib:notify",
		src,
		{ type = "success", description = "Electric charger deployed successfully!" }
	)
end)
...
RegisterNetEvent("LNS_Fuel:removeCharger", function(stationId, chargerIndex)
	...
	local plyId = getPlayerIdentifier(src)
	local station = ownedStations[stationId]
	if not station or not station.electric_chargers or station.pending then
		return
	end
	...
	if not isNearStationManagement(src, stationId, 20.0) then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s attempted to remove charger from station %s too far away!"):format(
				src,
				stationId
			)
		)
	...
	lib.logger(
		src,
		"Station Charger Removed",
		("Dismantled electric charger #%d at station %s"):format(index, stationId),
		("stationId:%s"):format(stationId),
...
lib.callback.register("LNS_Fuel:spawnDeliveryVehicles", function(source)
	...
	if not activeDelivery then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s requested server-side vehicle spawn via LNS_Fuel:spawnDeliveryVehicles without an active delivery run!"):format(
				src
			)
		)
		return nil
	...
	if activeDelivery.truckNetId or activeDelivery.trailerNetId then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s requested LNS_Fuel:spawnDeliveryVehicles but vehicles are already spawned!"):format(
				src
			)
		)
		return nil
	...
	local trailer =
		CreateVehicleServerSetter(trailerModel, "trailer", trailerPos.x, trailerPos.y, trailerPos.z, trailerPos.w)
...
RegisterNetEvent("LNS_Fuel:giveDeliveryKeys", function(plate, vehNetId)
	...
	if Settings.spawnType == "server" then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s requested delivery keys via client event but spawnType is server!"):format(src)
		)
		return
	end
	...
	if not activeDelivery then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s requested delivery keys via LNS_Fuel:giveDeliveryKeys without an active delivery run!"):format(
				src
			)
		)
		return
	...
	if activeDelivery.truckNetId then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s requested delivery keys but already has spawned truck/keys!"):format(src)
		)
		return
	end
	...
	if DoesEntityExist(vehicle) then
		...
		if model ~= expectedModel then
			reportSecurityCheck(
				src,
				("[Security Check] Player %s requested delivery keys for non-delivery vehicle model! Model: %s, Expected: %s"):format(
					src,
					model,
					expectedModel
				)
		...
		reportSecurityCheck(
			src,
			("[Security Check] Player %s requested delivery keys for a vehicle that does not exist on the server!"):format(
				src
			)
		)
	end
...
RegisterNetEvent("LNS_Fuel:hireEmployee", function(stationId, targetServerId)
	...
	if not isNearStationManagement(src, stationId, 20.0) then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s attempted to hire employee for station %s too far away!"):format(
				src,
				stationId
			)
		)
	...
	if not targetPed or targetPed == 0 then
		return TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "error", description = locale("notify_player_not_found") }
		)
	end
	...
	if not targetId then
		return TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "error", description = locale("notify_cannot_identify_player") }
		)
	end
	...
	if station.owner == targetId then
		return TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "error", description = locale("notify_cannot_hire_self") }
		)
	end
	...
	for _, emp in ipairs(station.employees) do
		if emp.identifier == targetId then
			return TriggerClientEvent(
				"ox_lib:notify",
				src,
				{ type = "error", description = locale("notify_player_already_employed") }
			)
		end
	end
	...
	lib.logger(
		src,
		"Station Employee Hired",
		("Hired employee %s (identifier: %s) for station %s"):format(targetName, targetId, stationId),
		("stationId:%s"):format(stationId),
		("employeeName:%s"):format(targetName),
		("employeeIdentifier:%s"):format(targetId)
	)

	TriggerClientEvent(
		"ox_lib:notify",
		src,
		{ type = "success", description = locale("notify_employee_hired"):format(targetName) }
	)
	TriggerClientEvent(
		"ox_lib:notify",
		validatedTarget,
		{ type = "info", description = locale("notify_hired_by_station"):format(station.name) }
	)
end)

RegisterNetEvent("LNS_Fuel:fireEmployee", function(stationId, employeeIdentifier)
	local src = source
	if type(stationId) ~= "string" or type(employeeIdentifier) ~= "string" then
		return
	end
	if not isOwner(src, stationId) then
		return
	...
	if not isNearStationManagement(src, stationId, 20.0) then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s attempted to fire employee from station %s too far away!"):format(
				src,
				stationId
			)
		)
	...
	local station = ownedStations[stationId]
	if not station or not station.employees or station.pending then
		return
	end
	...
	if not foundIndex then
		return TriggerClientEvent(
			"ox_lib:notify",
			src,
			{ type = "error", description = locale("notify_employee_not_found") }
		)
	end
	...
	MySQL.query.await(
		"DELETE FROM lns_fuel_employees WHERE station_id = ? AND identifier = ?",
		{ stationId, employeeIdentifier }
	)
	...
	lib.logger(
		src,
		"Station Employee Fired",
		("Fired employee %s (identifier: %s) from station %s"):format(employeeName, employeeIdentifier, stationId),
		("stationId:%s"):format(stationId),
		("employeeName:%s"):format(employeeName),
		("employeeIdentifier:%s"):format(employeeIdentifier)
	)

	TriggerClientEvent(
		"ox_lib:notify",
		src,
		{ type = "success", description = locale("notify_employee_fired"):format(employeeName) }
	)
	...
	if GetResourceState("qbx_core") == "started" then
		...
		if tPlayer then
			TriggerClientEvent(
				...
				{ type = "warning", description = locale("notify_employee_let_go"):format(station.name) }
			)
		end
