...
local function applyFuelLevel(networkId, fuelLevel)
	...
	if veh == 0 or GetEntityType(veh) ~= 2 then
		return
	end
...
local function isNearAnyPump(playerCoords, maxDistance)
	...
	if not Settings or not Settings.Stations then
		return false
	end
	for i = 1, #Settings.Stations do
...
RegisterNetEvent("LNS_Fuel:pay", function(cost, currentFuel, nId, stationId, litersFueled)
	...
	if not validatedCost or not validatedCurrentFuel or not validatedNId or not validatedLiters then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s sent invalid/NaN/inf pay parameters! Cost: %s, Fuel: %s, NetId: %s, Liters: %s"):format(
				src,
				tostring(cost),
				tostring(currentFuel),
				tostring(nId),
				tostring(litersFueled)
			)
		)
		return
	end
	...
	if stationId ~= nil then
		if type(stationId) ~= "string" then
			reportSecurityCheck(
				src,
				("[Security Check] Player %s sent invalid type for stationId: %s"):format(src, type(stationId))
			)
			return
		end
		if not GetStationIndex(stationId) then
			reportSecurityCheck(
				src,
				("[Security Check] Player %s sent unregistered stationId: %s"):format(src, stationId)
			)
			return
		end
	end
	...
	if cost < 0 or litersFueled < 0 or currentFuel < 0 or currentFuel > 100 then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s sent invalid/out-of-bounds pay parameters! Cost: %s, Liters: %s, Fuel: %s"):format(
				src,
				cost,
				litersFueled,
				currentFuel
	...
	if vehicle == 0 or not DoesEntityExist(vehicle) then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s triggered LNS_Fuel:pay with invalid vehicle NetId!"):format(src)
		)
		return
	end
	...
	if distanceToVeh > 15.0 then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s triggered LNS_Fuel:pay too far from vehicle! Distance: %0.2f meters"):format(
				src,
				distanceToVeh
			)
		)
	...
	if not isNearAnyPump(playerCoords, 15.0) then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s triggered LNS_Fuel:pay too far from any gas pump!"):format(src)
		)
		return
	end
	...
	if fuelDelta > 0 then
		if litersFueled < fuelDelta - 2.0 then
			reportSecurityCheck(
				src,
				("[Security Check] Player %s attempted free refuel! Liters Fueled: %s, Actual Fuel Delta: %0.2f (Server: %0.2f, Client Target: %s)"):format(
					src,
					litersFueled,
					fuelDelta,
					currentServerFuel,
	...
	local expectedPrice = isElectric and ((Settings.electricPriceTick or 2) / Settings.refillValue)
		or (Settings.priceTick / Settings.refillValue)
	...
	if cost < expectedCost - 2 then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s triggered LNS_Fuel:pay with manipulated cost! Cost: %s, Expected: %s, Liters: %s"):format(
				src,
				cost,
				expectedCost,
				litersFueled
	...
	if stationId and ownedStations and ownedStations[stationId] then
		...
		if not isElectric then
			...
			if fuelToDeduct <= 0 and litersFueled and litersFueled > 0 then
				return TriggerClientEvent(
					"ox_lib:notify",
					src,
					{ type = "error", description = locale("notify_station_out_of_fuel") }
				)
			end
		end

		if not processPayment(src, cost) then
			return
		end
		...
		lib.logger(
			src,
			"Fuel Purchase",
			("%s vehicle (netId: %d) with %0.2f %s for $%d at owned station %s. New charge level: %d%%"):format(
				actionWord,
				validatedNId,
				litersFueled,
				unitWord,
			...
			("netId:%d"):format(validatedNId),
			("liters:%0.2f"):format(litersFueled),
			("cost:$%d"):format(cost)
		)

		TriggerClientEvent("ox_lib:notify", src, {
			type = "success",
			description = isElectric and ("Charged to %s%% - $%s"):format(currentFuel, cost)
				or locale("fuel_success", currentFuel, cost),
		})
		return
	end
	...
	lib.logger(
		src,
		"Fuel Purchase",
		("%s vehicle (netId: %d) with %0.2f %s for $%d at standard station. New charge level: %d%%"):format(
			actionWord,
			validatedNId,
			litersFueled,
			unitWord,
...
RegisterNetEvent("LNS_Fuel:fuelCan", function(ownsCan, cost, stationId)
	...
	if type(ownsCan) ~= "boolean" then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s sent invalid type for ownsCan: %s"):format(src, type(ownsCan))
		)
		return
	end
	...
	if not validatedCost then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s sent invalid/NaN/inf Jerry Can price: %s"):format(src, tostring(cost))
		)
		return
	end
	cost = validatedCost
	...
	if stationId ~= nil then
		...
		if not GetStationIndex(stationId) then
			reportSecurityCheck(
				src,
				("[Security Check] Player %s triggered LNS_Fuel:fuelCan with unregistered stationId: %s"):format(
					src,
					stationId
				)
			)
	...
	if not isNearAnyPump(playerCoords, 15.0) then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s triggered LNS_Fuel:fuelCan too far from any gas pump!"):format(src)
		)
		return
	end
	...
	if cost < expectedCost then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s attempted Jerry Can refill/purchase with manipulated cost! Cost: %s, Expected: %s"):format(
				src,
				cost,
				expectedCost
			)
	...
	if ownsCan then
		...
		if stationId and ownedStations and ownedStations[stationId] then
			...
			station.balance = station.balance + cost
			MySQL.query.await(
				"UPDATE lns_fuel_stations SET balance = ? WHERE station_id = ?",
				{ station.balance, stationId }
			)
			TriggerClientEvent("LNS_Fuel:syncStation", -1, stationId, station)
		end
		...
		lib.logger(
			src,
			"Jerry Can Transaction",
			("Refilled Petrol Can for $%d at station %s"):format(cost, stationId or "standard"),
			stationId and ("stationId:%s"):format(stationId) or nil,
			("cost:$%d"):format(cost)
		)
		...
		lib.logger(
			src,
			"Jerry Can Transaction",
			("Purchased new Petrol Can for $%d at station %s"):format(cost, stationId or "standard"),
			stationId and ("stationId:%s"):format(stationId) or nil,
			("cost:$%d"):format(cost)
		)
...
RegisterNetEvent("LNS_Fuel:updateFuelCan", function(dura, nId, currentFuel)
	...
	if not validatedDura or not validatedNId or not validatedCurrentFuel then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s sent invalid/NaN/inf updateFuelCan parameters! Dura: %s, NetId: %s, Fuel: %s"):format(
				src,
				tostring(dura),
				tostring(nId),
				tostring(currentFuel)
	...
	if distanceToVeh > 15.0 then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s triggered LNS_Fuel:updateFuelCan too far from vehicle! Distance: %0.2f meters"):format(
				src,
				distanceToVeh
			)
		)
	...
	if not wpnItem or wpnItem.name ~= "WEAPON_PETROLCAN" then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s triggered LNS_Fuel:updateFuelCan without a Petrol Can equipped!"):format(src)
		)
		return
	end
	...
	if fuelAdded > 0 then
		...
		if dura < minAllowedDura then
			reportSecurityCheck(
				src,
				("[Security Check] Player %s attempted LNS_Fuel:updateFuelCan with manipulated durability! Dura: %s, Expected Min: %0.2f, Fuel Added: %0.2f"):format(
					src,
					dura,
					minAllowedDura,
					fuelAdded
	...
	lib.logger(
		src,
		"Jerry Can Refuel",
		("Refueled vehicle (netId: %d) using Jerry Can. New fuel level: %d%%"):format(validatedNId, currentFuel),
		("netId:%d"):format(validatedNId),
		("newFuel:%d"):format(currentFuel)
	)
end)
...
RegisterNetEvent("LNS_Fuel:initVehicleFuel", function(vehNetId, fuelLevel)
	...
	if #(playerCoords - vehCoords) > 15.0 then
		reportSecurityCheck(
			src,
			("[Security Check] Player %s attempted LNS_Fuel:initVehicleFuel too far from vehicle! Distance: %0.2f meters"):format(
				src,
				#(playerCoords - vehCoords)
			)
		)
		return
	end
...
RegisterNetEvent("LNS_Fuel:setVehicleFuel", function(vehNetId, fuelLevel)
	...
	if vehSeat ~= vehicle then
		...
		if #(playerCoords - vehCoords) > 15.0 then
			reportSecurityCheck(
				src,
				("[Security Check] Player %s attempted LNS_Fuel:setVehicleFuel too far from vehicle! Distance: %0.2f meters"):format(
					src,
					#(playerCoords - vehCoords)
				)
			)
...
lib.addCommand("setfuel", {
	...
	lib.logger(
		source,
		"Set Fuel Admin Command",
		("Admin set vehicle fuel level to %d%%"):format(amount),
		("newFuel:%d"):format(amount)
	)
...
CreateThread(function()
	...
	PerformHttpRequest(
		"https://raw.githubusercontent.com/LumaNodeStudios/LNS_Fuel/main/fxmanifest.lua",
